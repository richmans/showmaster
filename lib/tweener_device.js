...
TweenerDevice.prototype.activateTweener = function() {
  if(this.tweenConnection ==  undefined) {
    this.showmaster.createConnection("tween", "output", this.id, "tween")
  }
}
...
TweenerDevice.prototype.handleInput = function(value)  {
  ...
  } else if (this.targetValue != this.outputPorts["output"].lastValue) {
    this.activateTweener()
  }
}

TweenerDevice.prototype.handleTween = function(value) {
  var currentValue = this.outputPorts["output"].lastValue
  var newValue
  ...
  this.outputs["output"].update(newValue)
}
